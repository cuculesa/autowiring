...
/// </remarks>
class ExpressPacket:
  public std::enable_shared_from_this<ExpressPacket>,
  public TeardownNotifier
{
public:
  ExpressPacket(const ExpressPacket& rhs) = delete;
  ExpressPacket(ExpressPacket&&) = delete;

  // Must hold the lock to 'factory' when calling this constructor
  ExpressPacket(AutoPacketFactory& factory, std::shared_ptr<void>&& outstanding);
  ~ExpressPacket();
...
protected:
  ...
  /// </summary>
  std::shared_ptr<ExpressPacket> SuccessorUnsafe(void);
...
public:
  ...
  /// </returns>
  /// <remarks>
  /// Although "ExpressPacket &" and "const ExpressPacket&" argument types will be
  /// satisfied, the ExpressPacket does not "have" these types.
  /// </remarks>
  template<class T>
  bool Has(int tshift=0) const {
    std::lock_guard<std::mutex> lk(m_lock);
    return HasUnsafe(DecorationKey(auto_id_t<T>::key(), tshift));
  }
  ...
  const T& Get(int tshift=0) const {
    static_assert(!std::is_same<T, AnySharedPointer>::value, "AnySharedPointer is not permitted to be directly decorated on an ExpressPacket");

    const T* retVal;
    if (!Get(retVal, tshift))
      ThrowNotDecoratedException(DecorationKey(auto_id_t<T>::key(), tshift));
    return *retVal;
  }
  ...
  bool Get(const T*& out, int tshift=0) const {
    DecorationKey key(auto_id_t<T>::key(), tshift);
  ...
  bool Get(const std::shared_ptr<T>*& out, int tshift=0) const {
    static_assert(std::is_const<T>::value, "Cannot get a non-const shared pointer from ExpressPacket, declare as `const std::shared_ptr<const T>*`");
    ...
    DecorationKey key(auto_id_t<TActual>::key(), tshift);
    const DecorationDisposition* pDisposition = GetDisposition(key);
    if (!pDisposition) {
      out = nullptr;
      return false;
  ...
  bool Get(std::shared_ptr<const T>& out, int tshift = 0) const {
    std::lock_guard<std::mutex> lk(m_lock);
    auto deco = m_decoration_map.find(DecorationKey(auto_id_t<T>::key(), tshift));
  ...
  std::unique_ptr<const T*[]> GetAll(int tshift = 0) const {
    ...
    auto q = m_decoration_map.find(DecorationKey(auto_id_t<T>::key(), tshift));
  ...
  std::unique_ptr<std::shared_ptr<const T>[]> GetAllShared(int tshift = 0) const {
    ...
    // If decoration doesn't exist, return empty null-terminated buffer
    auto q = m_decoration_map.find(DecorationKey(auto_id_t<TActual>::key(), tshift));
    if (q == m_decoration_map.end())
  ...
  void MarkUnsatisfiable(void) {
    MarkUnsatisfiable(DecorationKey(auto_id_t<T>::key(), 0));
  }
  ...
  const T& Decorate(T t) {
    ...
    Decorate(
      ...
      DecorationKey(auto_id_t<T>::key(), 0)
    );
    return *ptr;
  }
  ...
  void Decorate(const std::shared_ptr<T>& ptr) {
    DecorationKey key(auto_id_t<T>::key(), 0);
    ...
    static_assert(!std::is_same<T, ExpressPacket>::value, "Cannot decorate a packet with another packet");
  ...
  void DecorateImmediate(const T& immed, const Ts&... immeds) {
    ...
    DecorationDisposition* pTypeSubs[1 + sizeof...(Ts)] = {
      &DecorateImmediateUnsafe(DecorationKey(auto_id_t<T>::key(), 0), &immed),
      &DecorateImmediateUnsafe(DecorationKey(auto_id_t<Ts>::key(), 0), &immeds)...
    };
    ...
    MakeAtExit([this, &pTypeSubs] {
      ...
      for (const std::type_info* ti : {&auto_id_t<T>::key(), &auto_id_t<Ts>::key()...})
  ...
  /// <summary>
  /// Convenience overload, provided to allow the attachment of receive-only filters to a const ExpressPacket
  /// </summary>
  template<class Fx>
  const ExpressPacket& operator+=(Fx&& fx) const {
    static_assert(
      !Decompose<decltype(&Fx::operator())>::template any<arg_is_out>::value,
      "Cannot add an AutoFilter to a const ExpressPacket if any of its arguments are output types"
      );
    *const_cast<ExpressPacket*>(this) += std::forward<Fx&&>(fx);
    return *this;
  }
  ...
  /// </remarks>
  template<class T>
  inline const SatCounter& GetSatisfaction(void) const { return GetSatisfaction(auto_id_t<T>::key()); }

  /// <summary>
  /// Returns the next packet that will be issued by the packet factory in this context relative to this context
  /// </summary>
  std::shared_ptr<ExpressPacket> Successor(void);

  /// <returns>True if the indicated type has been requested for use by some consumer</returns>
  template<typename T>
  bool HasSubscribers(void) const {
    return HasSubscribers(DecorationKey{auto_id_t<T>::key(), 0});
  }

  /// <returns>Zero if there are no publishers, otherwise the number of publishers</returns>
  template<typename T>
  size_t HasPublishers(void) const {
    return HasPublishers(DecorationKey{auto_id_t<T>::key(), 0});
  }

  struct SignalStub {
    SignalStub(const ExpressPacket& packet, std::condition_variable& cv) :
      packet(packet),
      cv(&cv)
    {}

    const ExpressPacket& packet;
...
/// <summary>
/// ExpressPacket specialization
/// </summary>
/// <remarks>
/// Because this type is immediately satisfied, it is neither an input nor an output
/// </remarks>
template<>
class auto_arg<ExpressPacket&>
{
public:
  typedef ExpressPacket& type;
  typedef ExpressPacket& arg_type;
  typedef ExpressPacket id_type;
  ...
  static ExpressPacket& arg(ExpressPacket& packet) {
    return packet;
  }
};
